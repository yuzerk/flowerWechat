...
Page({
  data: {
    ...
    nowtime:0,
    // type 购买类型 1-团购 2-秒杀 0-加入购物车正常购买  
    type:0,
    ntype:'',
    groupprice:0,
    groupNum:0,
  ...
  addToCart: function() {
    ...
    if (that.data.openAttr == false) {
      ...
      util.request(api.CartAdd, {
        commodityId: that.data.goods.id,
        commodityCount: that.data.number}, "POST")
        // productId: that.data.proId ? that.data.proId : that.data.productList[0].id}, "POST")
        .then(function(res) {
          wx.hideLoading();
          let _res = res;
          if (_res.code === 200) {
            ...
            that.setData({
              openAttr: !that.data.openAttr,
              cartGoodsCount: _res.data.count
            });
            if (that.data.userHasCollect == 1) {
              that.setData({
                'collectBackImage': that.data.hasCollectImage
            ...
            wx.getUserInfo({
              success: function (res) {
                ...
                user.loginByWeixin(res).then(res => {
                  ...
                  app.globalData.token = res.data.appToken;
                  that.cartGoodsCount();
                }).catch((err) => {
                  console.log(err)
                });
